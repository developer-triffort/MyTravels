...
- (id) initWithEngine: (SA_OAuthTwitterEngine *) engine andOrientation:(UIInterfaceOrientation)theOrientation {
	if (self = [super init]) {
		...
		if (UIInterfaceOrientationIsLandscape( self.orientation ) )
			_webView = [[UIWebView alloc] initWithFrame: CGRectMake(0, 32, 480, 188)];
		else
...
- (void) loadView {
    ...
    [button setImage:[UIImage imageNamed:@"CancelButton.png"] forState:UIControlStateNormal];
    //[button setTitle:@"Cancle" forState:UIControlStateNormal];
    [button addTarget:self action:@selector(cancel:) forControlEvents:UIControlEventTouchUpInside];
                                           
    [button setFrame:CGRectMake(0, 0, 62, 30)];
